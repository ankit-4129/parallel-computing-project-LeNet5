...
#define FILE_TRAIN_IMAGE		"./fashion_dataset/train-images-idx3-ubyte"
#define FILE_TRAIN_LABEL		"./fashion_dataset/train-labels-idx1-ubyte"
#define FILE_TEST_IMAGE		"./fashion_dataset/t10k-images-idx3-ubyte"
#define FILE_TEST_LABEL		"./fashion_dataset/t10k-labels-idx1-ubyte"
...
void load_train_test()
{
	...
	/*
	for (int i = 10; i < 20; i++) {
		display_img(test_data[i],1);
	}
	/**/
	LeNet5 *lenet = (LeNet5 *)malloc(sizeof(LeNet5));
	if (load(lenet, LENET_FILE)) {
		printf("Model not found, Setting random weights\n");
		Initial(lenet);
	}
	clock_t start = clock();
	int batches[] = { 300 };
	int n = sizeof(batches) / sizeof(*batches);
	for (int i = 0; i < n; ++i)
