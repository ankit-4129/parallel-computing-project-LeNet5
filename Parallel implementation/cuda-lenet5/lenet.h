...
#define TRAIN_KENREL_THREADS 1024
...
//len0 = len1 = 2 , always dont parallalize this
...
__global__ void train_kernel(LeNet5* lenet, image* inputs, uint8* labels, int startidx, int batchSize, double* buffer) {
	...
	//critical section for blocks
	{
		//every block needs to do this
		//buffer[] is shared among blocks
		//buffer[] needs to be filled parallel with threads
		int iter_tidx = tidx;
		while (iter_tidx < GETCOUNT(LeNet5)) {
			buffer[iter_tidx] += ((double*)deltas)[iter_tidx]; //atomic add system
			iter_tidx += TRAIN_KENREL_THREADS;
		}
	}
...
void TrainBatch(LeNet5* lenet, image* inputs, uint8* labels, int startidx, int batchSize)
{
	...
	train_kernel <<<batchSize, TRAIN_KENREL_THREADS >>> (lenet, inputs, labels, startidx, batchSize, buffer);
